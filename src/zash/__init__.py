...
def main() -> int:
    ...
    logger = get_logger("zash.main")

    # Tries to set the process title and logs failures
    try:
        import setproctitle

        setproctitle.setproctitle("zash")
        logger.info("Process title set to 'zash'.")
    except Exception as e:
    ...
    parser = argparse.ArgumentParser(
        prog="zash",
        description=_(
            "Zash - A modern terminal emulator with session management"
        ),
        epilog=_("For more information, visit: https://github.com/leoberbert/zash"),
    )
    ...
    try:
        ...
        alert.set_detail(_("Could not start Zash.\n\nError: {}").format(e))
