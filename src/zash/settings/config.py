...
class AppConstants:
    ...
    APP_ID = "io.github.leoberbert.zash"
    APP_TITLE = "Zash"
    APP_VERSION = "1.8.4"
    DEVELOPER_NAME = "Zash Project"
    DEVELOPER_TEAM = ["Zash Maintainers"]
    COPYRIGHT = "© 2025 Zash"
    WEBSITE = "https://github.com/leoberbert/zash"
    ISSUE_URL = "https://github.com/leoberbert/zash/issues"
...
class ConfigPaths:
    ...
    def __init__(self):
        self.logger = get_logger("zash.config.paths") if UTILS_AVAILABLE else None
        self._setup_paths()
    ...
    def _get_cache_directory(self) -> Path:
        if xdg_cache := os.environ.get("XDG_CACHE_HOME"):
            return Path(xdg_cache) / "zash"
        return Path.home() / ".cache" / "zash"

    def _use_fallback_paths(self):
        home = Path.home()
        self.CONFIG_DIR = home / ".config" / "zash"
        self.SESSIONS_FILE = self.CONFIG_DIR / "sessions.json"
        self.SETTINGS_FILE = self.CONFIG_DIR / "settings.json"
        self.STATE_FILE = self.CONFIG_DIR / "session_state.json"
        self.LAYOUT_DIR = self.CONFIG_DIR / "layouts"
        self.CACHE_DIR = home / ".cache" / "zash"
        self.LOG_DIR = self.CONFIG_DIR / "logs"
...
class DefaultSettings:
    ...
    def get_defaults() -> Dict[str, Any]:
        return {
            ...
            # Icon Theme Strategy: "zash" (bundled) or "system"
            # Using Zash Icons by default speeds up GTK4 startup
            "icon_theme_strategy": "zash",
            # Shortcuts
            "shortcuts": {
...
def initialize_configuration():
    ...
    logger = get_logger("zash.config") if UTILS_AVAILABLE else None
    try:
        if logger:
            logger.info(
...
try:
    ...
    CONFIG_DIR = os.path.expanduser("~/.config/zash")
