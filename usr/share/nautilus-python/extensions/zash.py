...
"""
Zash - Nautilus Extension
Adds context menu options to open Zash. Supports local paths,
...
try:
    ...
    print(f"Zash Extension: Could not set up localization: {e}")

_ = gettext.gettext

# --- Constants ---
TERMINAL_EXECUTABLE = shutil.which("zash")
...
class ZashinalExtension(GObject.GObject, Nautilus.MenuProvider):
    ...
    def _launch(self, command: list[str]):
        ...
        if not TERMINAL_EXECUTABLE:
            print("Zash Error: 'zash' executable not found.")
            return
        try:
    ...
    def _launch_local_session(self, menu_item: Nautilus.MenuItem, files: list[Nautilus.FileInfo]):
        """
        Opens Zash in the specified local or GVFS directory path.
        """
        file = files[0]
        local_path = self._get_local_path(file)
        if not local_path:
            print(f"Zash Error: Could not get local path for {file.get_uri()}")
            return
    ...
    def _launch_remote_ssh_session(self, menu_item: Nautilus.MenuItem, files: list[Nautilus.FileInfo]):
        """
        Parses a remote URI and launches Zash with a direct SSH connection.
        """
        file = files[0]
        uri = file.get_uri()
        try:
            ...
            self._launch(cmd)
            print(f"Zash: Launched SSH session to {target}")
    ...
    def _get_menu_items(self, files: list[Nautilus.FileInfo]) -> list[Nautilus.MenuItem]:
        ...
        # Case 2: Local location (file://)
            ...
            local_item = Nautilus.MenuItem(
                ...
                label=_("Open in Zash"),
                tip=_("Open {} in Zash").format(file.get_name()),
            )
